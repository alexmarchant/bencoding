...
func TestParseString(t *testing.T) {
	...
	in := "4:test"
	if x, e := ParseString(in); x[0].StringValue != "test" ||
		e != nil {
	...
	in = "i16e"
	if x, e := ParseString(in); x[0].IntValue != 16 ||
		e != nil {
	...
	in = "l4:testi16ee"
	if x, e := ParseString(in); x[0].ListValue[0].StringValue != "test" ||
	...
	if x, e := ParseString(in); x[0].DictValue["meaning-of-life"].IntValue != 42 ||
		e != nil {
	...
	in = "d15:meaning-of-lifei42ee"
	if x, e := ParseString(in); x[0].UnparsedString != in ||
		e != nil {
	...
	if x, e := ParseString(in); x[0].DictValue["bencoding-data-types"].ListValue[0].StringValue != "string" ||
	...
	in = string(fileBytes)
	if x, e := ParseString(in); x[0].DictValue["announce"].StringValue != "http://files.publicdomaintorrents.com/bt/announce.php" ||
